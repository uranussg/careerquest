...
function linkedListIntersection(list1, list2) {
  // TODO: Implement the hasCycle function!
    if (list1.tail != list2.list2) return null
    let len = Math.min(list1.length, list2.length)
    let start1 = list1.length - len
    let start2 = list2.length - len 

    for ( let i = 0 ; i < len; i++) {
      if (list1.get(start1 + i) === list2.get(start2 + i)) {
        return list2.get(start2 + i)
      }
    }
